...
> **How this phase actually went.** The spikes were **not run as spikes**. The change was built
> straight from the artefacts, and each unknown was settled afterwards on the built article — which
> worked, but is not what this phase was for, and two of them turned out to matter. See DECISIONS.md
> D-041 (`collapsible` is ignored on a `MultiColumnSection`), D-043 (the pinned header does hold),
> D-046 (the plan's way of addressing a Conversation row does not work) and D-048 (the 🛑 expression,
> verbatim). Ticked below where the answer is now known, however it was reached.

- [x] **Spike the custom screen element.** Add a throwaway `CustomScreenElement` to
  ...
  Screenshot into `tmp/`.
- [x] **Spike the pinned header.** Extend that same throwaway element into a `height`-bounded box with
  ...
  on it.
- [x] **Spike the 🛑 in a model file.** Add the `ExpressionColumn` to `Conversation_OM` with the emoji
  ...
  - [x] Record the working expression **verbatim** in `architecture.md`'s seam 3. No artifact currently
    contains the string an implementer would paste, only its semantics — and phase E depends on it.
  - [x] If the model checker rejects the glyph, or it renders as a replacement character, fall back to
...
- [x] **Read a working descriptor out of the store — the check everything else rests on.** With the
...
- [x] **Spike the read-by-id.** In a scratch component, read one `OpenQuestion_DM` document by ThingID
...
- [x] **Confirm the value accessor.** Establish whether `@com.mgmtp.a12.kernel/kernel-md-facade` gives
...
- [x] Revert every spike in this phase. Nothing from it is kept except the notes and the answers.
...
- [x] Capture a fixture: a real `Conversation_DM` JSON document with a full invoice-slice transcript —
  ...
  stack, not by hand.
- [x] Write `client/src/test/components/conversation/entries.test.ts` first: reading the fixture yields
...
- [x] Implement `client/src/components/conversation/entries.ts` to satisfy it. Only the fields the
  Transcript renders — `seq`, `at`, `role`, `kind`, `text`, `toolName`, `toolArgs`, `toolResult`,
  `questionId`, `promptTokens`, `completionTokens`.
- [x] Write `speaker.test.ts` first, one case per row of domain.md's Speaker table, including: an
...
- [x] Implement `speaker.ts`.
- [x] Write the clustering test: Entries group into clusters, and a separator is due when the day
...
- [x] Implement the clustering in `entries.ts`, using `date-fns` for the separator's text.
- [x] Write `cost.test.ts` first: the fixture's tokens sum to the expected figure; Entries without
...
- [x] Write `subject.test.ts` first, one case each: each of the four `TRIGGER_ELIGIBLE_MODELS` yields
...
- [x] `just test-client` green.

## C — The Transcript, rendered

- [x] Add `"conversation-transcript"` to `WidgetAnnotationValue` in `components/widgetAnnotation.ts`.
- [x] Add `client/src/components/CustomScreenElements.tsx`: the `formModelMap.CustomScreenElement`
...
- [x] Register it in `appsetup.ts` by spreading, beside the existing `Control` override.
- [x] Write component tests (`@testing-library/react`, as the markdown editor's tests do) over the
...
- [x] Implement `Bubble.tsx`, `Receipt.tsx`, `icons.ts`, `ConversationTranscript.tsx`. Styling via
...
- [x] Implement `client/src/sagas/openForeignForm.ts` per seam 2b, and register it in `appsetup.ts`'s
  ...
  - [x] Unit-test the descriptor it builds — a bare ThingID in, a docRef out, `model` always present,
    `masterModule` honoured — and that a vetoed cancel dispatches no push at all.
- [x] Write the Header's component tests first: it names the Assistant and the title; it shows 🛑 when
...
- [x] Replace the Entries `InlineRepeat` in `Conversation_FM` with the `CustomScreenElement`
  ...
  `f_entries`.
- [x] Set `collapsible: true, initiallyCollapsed: true` on `Conversation_FM`'s `ConversationHeader`
...
- [x] Teach `import/validate-models.mjs` the `exposes` rule, both halves: a `CustomScreenElement`
...
- [x] Correct that file's `INTENTIONALLY_UNEXPOSED` comment for `f_maxTurns`: the Header shows
...
- [x] `just test-models` green with **no new warnings** (12 would otherwise appear for the Entry
...
- [x] Click the header's *about* link and land on the subject Thing's form. Then open a Conversation
...
- [x] Implement `useThingById` per phase A's answer, read-only, failing soft, composing the docRef from
...
- [x] Test the question form's degraded state, since that is the one where the fetched document *is* the
...
- [x] Write the test first: given a Conversation with `currentQuestionId` and a loaded OpenQuestion,
...
- [x] Implement `PendingQuestion.tsx`. The prompt renders through the read-only
...
- [x] Press the landed form's `Cancel` and confirm it returns to the **Conversations** overview, not to
  a blank region — `empty-div` there means the master push in step 2 is missing or misparented.
- [x] Type into the answer field, then `Cancel`, and confirm the dirty-handling dialog *does* appear.
  It proves the veto path is live on the new route, which is the whole reason answering stayed on this
  form.
- [x] Add the same `CustomScreenElement` to `OpenQuestion_FM`, above `section_answer`, **carrying
...
- [x] Split `OpenQuestion_FM`'s `SectionQuestion`: `grid_prompt` stays in it and stays open; the four
...
- [x] `AssistantsAppModel_AM`: delete `OpenQuestionModule.menu`; set
  `content.initialActivity.descriptor.module` to `Conversation`. Leave both flows and all scenes.
- [x] Add the 🛑 `ExpressionColumn` to `Conversation_OM` (phase A settled its form), with
  `rowActionGroup.actions` untouched and the `Waiting for` reference column kept beside it.
- [x] `e2e/tests/base/2-navigation.spec.ts`: drop the *Open Questions* row from `MODULES`. Eight
  ...
  *Operations* module after this plan was written — each still checked against a column only its own
  overview declares.
- [x] `e2e/tests/base/7-forms-open.spec.ts`: drop `"Open Questions"` from its `MODULES` list. It
...
- [x] `e2e/tests/base/5-localization.spec.ts`: both locale tests assert the **welcome page's**
...
- [x] `e2e/tests/flow/2-restart.spec.ts`: `loginFreshly` waits for
...
- [x] `e2e/pages/OpenQuestionPage.ts`: rewrite `openQuestion()` to route through **Conversations**,
  ...
  - [x] ~~Search the **subject** ThingID, narrowed by the `Assistant key` column.~~ **Settled
  ...
  - [x] ~~Add `subjectThingId` to `RaisedQuestion` in `e2e/utils/agents.ts`.~~ Not needed —
    `RaisedQuestion.thingId` is the id the new address searches, and it was there already.
  - [x] Then press **Answer** on the Pending Question Bubble and assert the question form as it does
  ...
  - [x] Keep the rest of that doc comment's substance: it still only *fills in* fields, and it still
    leaves `answeredAt` empty on purpose.
  - [x] Add the test IDs the page object needs to `e2e/types/testIds.ts` and to the components:
    the transcript, a bubble, a receipt, the pending question, the answer button, the blocked marker,
    the header and its *about* link.
- [x] New `e2e/tests/base/9-conversation-transcript.spec.ts` — **9**, because `operations-as-things`
...
- [x] `specs/system/functional.md`: rewrite *Answering Open Questions* and *Watching an Assistant
  ...
  - [x] Correct *"Nothing adds them up — the transcript is the record"*. Something adds them up now:
...
- [x] `specs/system/architecture.md`: record the three new client seams — the custom screen element, the
...
- [x] `README.md` around line 185: the navigation list loses *Open Questions*, and *"Start at Open
  Questions"* becomes *"Start at Conversations — the rows marked 🛑 are waiting for you."*
- [x] `import/models/CONVENTIONS.md`: its overview section documents `content.columns[]` entries as
...
- [x] `docs/adr/0021-a-question-is-answered-in-its-conversation.md`, in the house form: context, the
...
- [x] `CONTEXT.md`: add **Transcript**, **Conversation Header**, **Speaker** and **Blocked** to the
  glossary if the terms are used outside this change's own documents — and only then.
- [x] Record the dormant models: `specs/system/functional.md` should say that the Open Questions
...
## Blocked at the end of the autonomous session

Everything above that needs a **running stack** is written and unrun, marked `[~]`. The stack's host
port forwarding failed VM-wide — the apps and Docker are healthy, the lima tunnel between macOS and
the guest is not — and the fix needs a Rancher Desktop restart that neither this session nor the peer
session managing the machine is permitted to take unilaterally. See **DECISIONS.md D-045**.

Green and committed without it: 401 client tests, 162 runtime, 78 integration, 29 models with 0
warnings, 13 validator checks, `just check`, `scripts/check-docs.mjs`.
