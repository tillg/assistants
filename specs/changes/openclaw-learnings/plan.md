...
- [x] Write the failing test first: a scripted transcript in which the model calls
...
- [x] Add `requiresApproval?: boolean` and `describeCall?(args): string` to `ToolDefinition` in
      `runtime/src/tools/registry.ts`.
- [x] Set both on `bookkeeping.postTransaction` in `runtime/src/tools/tools.ts`. Write the renderer
      so it reads as a sentence: amount, source, destination, date, subject.
- [x] Add `questionId?: string` to `Entry` in `runtime/src/domain/types.ts` and to
      ...
      appends.
- [x] Add the `approval-request` Entry kind, carrying `toolName`, `argsHash` and `questionId`.
- [x] Add `canonicalArgsHash(args)` — sorted keys, normalised numbers — with a test that two
      differently-ordered encodings of the same call hash equal.
- [x] Add `findApproval(conversation, operation, argsHash)` to `runtime/src/loop/advance.ts`: the
      walk-back from the architecture, returning *missing* / *waiting* / *declined* / *consumed* /
      *valid* rather than a boolean.
- [x] Call it in the tool-call loop, after the intent is written and before `tools.execute`. On
...
- [x] On *declined*, return `{ kind: "error", message: "The User declined this booking." }` and raise
      nothing.
- [x] Confirm the new test passes and the existing suspend/resume tests still do.
- [x] Add the refusal cases as tests: no request at all; `confirmed: false`; an approval already
      consumed by a previous call; and a second identical booking needing a second approval.
- [x] Add the drift test: the model re-issues the call with a changed amount after the yes, and is
      asked again rather than booking.
- [x] **Re-script the end-to-end invoice fixture.** Its model asks before booking and that ask no
      longer counts, so the transcript gains a refusal, an approval question and a resume.
- [x] Run `just test-models`, `just test-runtime`, then `just test-e2e` against the stack.
...
- [x] Update `specs/system/domain.md` (the rules), `specs/system/architecture.md` (the Tools table),
...
- [x] Add `usage?: { promptTokens: number; completionTokens: number }` to `LlmResponse` in
      `runtime/src/llm/provider.ts`.
- [x] Read it in `runtime/src/llm/openai.ts` and `runtime/src/llm/anthropic.ts`; return zeroes from
      `runtime/src/llm/scripted.ts`.
- [x] Add the two fields to `Entry` in `runtime/src/domain/types.ts` and to
      `import/models/conversation/Conversation_DM.json`, plus the form-model entry (ADR-0008). Not
      indexed — nothing queries them.
- [x] Write them onto **the first Entry the Turn wrote** in `advance()`: the `assistant` entry for a
      text reply, the first `tool-intent` otherwise.
- [x] Test: a Turn against a provider reporting usage records it; a tool-calling Turn records it on
      the first intent; a scripted Turn records zeroes; a Turn that errored records nothing.
- [x] Update `specs/system/domain.md` with the lower-bound caveat — a Turn that errored records no
      usage, so a Conversation's Turns sum to a lower bound on its cost, not its cost.
- [x] `just test-models` and `just test-runtime`.

## B — The seventh scan, quiet when idle (#2, #7, #11)

- [x] Add `cron-parser` as a runtime dependency, pinned per D-006.
- [x] Add `latestDueInstantBefore(now, cron, timezone)` with tests covering: never-yet-due, a normal
...
- [x] Add `SCHEDULE_TIMEZONE` to `runtime/src/config.ts` and `.env.example`.
- [x] Add indexed `scheduledFor` to `Conversation_DM` and its form model; add it to the
      `Conversation` type and to `SPECS` in `runtime/src/a12/things.ts`.
- [x] Add `conversationExistsFor(assistantKey, scheduledFor)` and
      `anyUnfinishedScheduledConversation(assistantKey)` beside the existing subject-Thing query in
      `runtime/src/watcher/watcher.ts`.
- [x] Add scan 7, honouring `paused`, `enabled` and the births-per-hour cap; skip on an unparseable
      cron, logging once per Assistant **per process** via an in-memory `Set`.
- [x] Test exactly-once across a re-scan, a restart and a replayed watermark; that three missed slots
      produce **one** Conversation; and that a slot is skipped while the previous one is unfinished.
- [x] Extend the scheduled birth prompt: if there is nothing to do, say so briefly and finish without
...
- [x] Test that a scheduled Conversation finding nothing ends `done` with no `OpenQuestion` created.
- [x] Give the Accountant a **daily** cron so the path is exercised — its *"chase what is unpaid"*
      Skill already exists and has no way to run.
- [x] **Rewrite that Skill to batch**: gather everything unpaid and raise **one** question covering
...
- [x] Update `specs/system/domain.md`, `functional.md`, `architecture.md` and `README.md`: a Schedule
      Trigger is no longer inert. **Six documents currently say it is** — `grep -rn` before claiming
      this is done.
- [x] `CONTEXT.md` — **already done: Schedule gained catch-up-once and stalls-rather-than-accumulates.**
...
- [x] Mark 1, 2, 6, 7 and 11 **built** in the learnings table, with their ADR numbers.
- [x] `just check` and the full `just test` with the stack up. Green: 26 models, 120 runtime units,
      68 live-stack integration tests, 288 client units, 30 end-to-end.
- [ ] `/spec:archive` — **left for the User deliberately.** The decisions taken unattended are in
      [DECISIONS-AUTONOMOUS.md](DECISIONS-AUTONOMOUS.md) in this directory and want reading before it
      is moved, and one of them (R-1) is a correction to `architecture.md` rather than a choice.
